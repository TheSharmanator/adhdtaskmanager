...
## Session 016 — 2026-06-15

**Type:** Polish fix + v2 merge to `main` + versioning
**Branch:** developed on `claude/adhd-taskmanager-review-zRJtI`, then merged to `main`
**Status:** Complete — `main` is now v2

### What Was Done

**Scheduled date added to the app task-title bracket**
The GCal event implies its date from the calendar grid, so its title only carries the time. In the app the only visible date was the *due* date, which can differ from the *scheduled* date — leaving the time bracket ambiguous. The `sched_bracket` computed in the index route now includes the day + date, e.g. `[Thu 18 Jun 09:00-10:00]`, shown on both the kiosk and mobile task cards. (`app.py`, commit `ad9c28a`.)

**Merged the v2 feature branch into `main`**
`main` had independently gained an **optional recurring end-date** feature (commit `1ad66e8`) that was not on the v2 branch. Merged `origin/main` into the feature branch and resolved the conflicts, keeping both feature sets:
- `recurring_templates.end_date` column + migration retained
- `check_recurring()`, the `/add` recurring handler, and `manage_recurring()` keep full end-date support (parse `recur_end_date`, store, display, and stop generating instances past the end date)
- The end-date guard was folded into **`complete_task_internal()`** so it coexists with the v2 refactor (where `complete_task` delegates to `complete_task_internal`); recurring tasks now stop regenerating once past their end date
- `add.html` keeps **both** the v2 duration/AI-estimate row **and** main's END DATE field

Verified before pushing: app boots; `/`, `/add`, `/manage_recurring` return 200 on desktop + mobile; `recur_end_date` field renders; no errors. Fast-forwarded `main` to the merge commit `c6c9255` and pushed.

**Versioning + branch hygiene**
- `v1` branch → `1ad66e8` (pre-merge snapshot / fallback)
- `v2` branch → `c6c9255` (= current `main`)
- BLUEPRINT.md marked **"Version: v2 (2.0) — Complete"**
- Redundant `backup/v1-pre-merge` branch deleted (was identical to `v1`)
- Feature branch `claude/adhd-taskmanager-review-zRJtI` kept as a merge safety net (at user's request)

> Note: this environment's git proxy blocks tag pushes and ref deletions, so version markers are kept as **branches** (not tags), and branch deletion was done by the user locally.

### Workflow change (effective next session)
**Rule 1: development now happens directly on `main`.** The feature branch is retained only as a safety net. `v1` remains the known-good fallback.

### Files Changed This Session
- `app.py` — date added to `sched_bracket`; recurring end-date conflict resolution; end-date guard in `complete_task_internal()`
- `templates/add.html` — merged duration/AI row + END DATE field
- `templates/manage_recurring.html` — end-date display (from main)
- `BLUEPRINT.md` — version label → v2 (2.0) Complete
- `PROGRESS.md` — this entry

### State at End of Session
- `main` = `v2` = commit `c6c9255`, pushed
- Mobile interface is the least-tested surface (smoke-tested only, not yet used on a real phone over Tailscale) — expect first real bugs there
- Next: collect bugs from a real production test run, then batch-fix

---

## Session 017 — 2026-06-16

**Type:** Feature additions + housekeeping  
**Branch:** `main` (Rule 1 — all development on main)  
**Status:** Complete — all changes pushed

### What Was Done

**1. Dedicated Voice Monkey device for Focus Mode (`VM_DEVICE_FOCUS`)**

All Focus Mode Voice Monkey announcements previously used the default alerts device. Added a separate `VM_DEVICE_FOCUS` config key so the user can route Focus Mode audio to a different Alexa device.

- `config.json` (local, gitignored) — added `"VM_DEVICE_FOCUS": "your-focus-device-name"`
- `config.json.example` — added `VM_DEVICE_FOCUS` and `USER_NAME` fields
- `app.py` — added `VM_DEVICE_FOCUS = config.get('VM_DEVICE_FOCUS', '')` at startup; wired `device=VM_DEVICE_FOCUS` into all four focus routes: `focus_start`, `focus_nudge`, `focus_break_reminder`, `focus_expired`

**2. Weekly tree feature removed**

The procedural SVG tree (oak/cherry/pine/willow/maple) was a gamification layer that added complexity without sufficient value.

Removed from `app.py`:
- `TREE_VARIANTS` constant
- `weekly_tree` table creation (existing table in live DB is harmlessly orphaned)
- `get_or_create_tree()`, `grow_tree()`, `maybe_reset_tree()` functions
- `maybe_reset_tree()` call in the background scheduler
- `tree = get_or_create_tree()` in index route + `tree=tree` from `render_template`
- `/api/tree` route
- `grow_tree()` call + `tree_level`/`tree_complete` from `complete_task_internal()` response

Removed from `templates/index.html`:
- `/* TREE */` CSS + `#tree-svg` rule
- `<svg id="tree-svg">` element
- `const treeData = {{ tree | tojson }};`
- `drawTree()` function + its call (~100 lines of SVG generation)

**3. Google Drive daily backup (`gdrive_backup.py`)**

New service backing up the full app folder to `My Drive/adhdtaskmanager/` at 02:00 every day. Keeps the last 7 backups.

- **`gdrive_backup.py`** (new file) — OAuth2 with `drive.file` scope; reads `gdrive_client_id`/`gdrive_client_secret` from `config.json`; tokens stored in DB as `gdrive_backup_*` keys (separate from GCal tokens); `_build_zip()` excludes `.git`, `__pycache__`, `*.pyc`, WAL files; `_get_or_create_folder()` finds/creates `adhdtaskmanager` folder in Drive root; `_cleanup_old_backups()` deletes oldest when >7 exist; `run_backup()` orchestrates the full upload
- **`app.py`** — `maybe_run_gdrive_backup()` fires at `02:00` daily (skips if already ran today); called from `background_task_checker()`; routes: `/gdrive_backup_auth_redirect`, `/gdrive_backup_callback`, `/gdrive_backup_disconnect`, `/api/gdrive_backup_status`, `/api/gdrive_backup_now`
- **`templates/settings.html`** — new **Google Drive Backup** section (between GCal Sync and ADHD Buffer): status dot, Connect button, Backup Now button, Disconnect, last backup timestamp; `loadGDriveStatus()` + `gdriveBackupNow()` JS functions

**Google Cloud Console one-time setup required (documented in README):**
1. Add `http://localhost:5001/gdrive_backup_callback` to Authorised Redirect URIs
2. Add your email as a Test User in the OAuth consent screen (required while app is in testing mode — omitting this gives a 403 `access_denied` error)

First live backup confirmed working.

**4. README updated**

- Added full **Google Drive Backup** setup section with exact redirect URI, test-user step, and daily-backup behaviour
- Removed stale weekly tree references from features list, architecture table, background thread description, and data model

**5. 10-minute gap between consecutive scheduled tasks**

Prevents ADHD burnout from back-to-back task scheduling.

- `scheduler.py` — added `GAP_MINS = 10` constant; when building the per-day busy list in `_find_slot()`, each already-allocated task slot has its end extended by `GAP_MINS` before being treated as busy time. GCal calendar events are unaffected (the gap only applies between tasks the scheduler itself places).

### Files Changed This Session
- `app.py` — VM_DEVICE_FOCUS wiring; tree feature removed; Drive backup scheduler hook + routes
- `config.json` (local only) — VM_DEVICE_FOCUS added
- `config.json.example` — VM_DEVICE_FOCUS + USER_NAME added
- `gdrive_backup.py` — new file
- `templates/index.html` — tree SVG + drawTree removed
- `templates/settings.html` — Google Drive Backup section added
- `scheduler.py` — GAP_MINS=10 inter-task gap
- `README.md` — Drive backup setup docs; tree refs removed

### State at End of Session
- All changes pushed to `main`
- Google Drive backup confirmed working in production (first backup completed)
- 10-minute scheduling gap active immediately on next GCal sync
- Next: continue live testing; collect and batch-fix any further bugs
