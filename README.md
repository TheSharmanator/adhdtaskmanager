...
- **Celebration finale** — completing a task triggers a full-screen celebration (randomised theme: matrix, glitch, gold-rush, fireworks, confetti) with applause audio and a Voice Monkey praise message.
...
| File | Responsibility |
|---|---|
| `app.py` | Main Flask app: routes, SQLite schema/migrations, background worker thread, Focus Mode, message bank, Voice Monkey, briefings, GCal sync orchestration, Google Drive backup scheduling. |
...
| `gdrive_backup.py` | Google Drive OAuth2 (`drive.file` scope), ZIP builder, folder management, upload, and cleanup. Keeps last 7 daily backups in `My Drive/adhdtaskmanager/`. |
...
A background thread (`background_task_checker`) runs every 60s to fire deadline nags, briefings, recurring-task generation, weekly message-bank generation, Google Calendar sync, and Google Drive backup (at 02:00 daily). It is guarded with `WERKZEUG_RUN_MAIN` so it only starts in the worker process under the Flask reloader.
...
In Settings → Google Drive Backup, connect your Google Drive account to enable automatic daily backups at 02:00. Backups are ZIPs of the full app folder (excluding `.git`, `__pycache__`, and SQLite WAL files), uploaded to **`My Drive/adhdtaskmanager/`**. The last 7 backups are kept; older ones are deleted automatically.

**One-time Google Cloud Console setup (same project as your Calendar credentials):**

1. In **APIs & Services → Credentials**, click your OAuth 2.0 Client ID and add the following to **Authorised redirect URIs**:
   ```
   http://localhost:5001/gdrive_backup_callback
   ```
   (Replace `5001` with your configured port if different.)

2. In **APIs & Services → OAuth consent screen**, scroll to **Test users** and add your Google account email. This is required while the app is in testing mode — without it Google will block the authorisation with a 403 error.

3. Back in the app, go to **Settings → Google Drive Backup** and click **CONNECT GOOGLE DRIVE BACKUP**.

Once connected, backups run automatically at 02:00 every day. Use **BACKUP NOW** in Settings to trigger an immediate backup at any time.
